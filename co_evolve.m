function [ rmse ] = co_evolve(data1, data2, data3, T, options)
...
filename = sprintf('./data/tmp/%s_T%d_k%d_fida%g_opttheta.mat', options.datasets, T, k, fida);

if exist(filename,'file')
    opttheta = importdata(filename);
else
    [opttheta, ~] = minFunc( @(p) graph_cost(p, k, alfa, beda, gama, T, n, fida, data1, data2, data3, zeros_idx), theta, opt);
...
% %% temp
% ground = (data1(T+1).mat + data2(T+1).mat);
% ground = ground + ground';
% ground = ground * ground;
% outputs = outputs + outputs';
% outputs = outputs * outputs;
% errors = ground - outputs;
% rmse = sqrt(sum(sum(errors .* errors))/length(nonzeros(ground)));
% rmse = full(rmse);
