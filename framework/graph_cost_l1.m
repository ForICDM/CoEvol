function [cost,grad] = graph_cost_l1(theta, k, alfa, beda, gama, T, n, fida, data1, data2, data3, zeros_idx)
...
weightdecay = alfa/2 * sum(sum(abs(U))) + beda/2 * sum( sum(abs(X1))) + gama/2 * sum( sum(abs(Y1))) ...
     + beda/2 * sum( sum(abs(X2))) + gama/2 * sum( sum(abs(Y2)));

if ~two_nets
    weightdecay = weightdecay + beda/2 * sum( sum(X3)) + gama/2 * sum( sum(Y3));
end

% gradient
Ugrad = Ugrad + alfa/2 * sign(U);
X1grad = X1grad + beda/2 * sign(X1);
X2grad = X2grad + beda/2 * sign(X2);
Y1grad = Y1grad + gama/2 * sign(Y1);
Y2grad = Y2grad + gama/2 * sign(Y2);
...
if ~two_nets
    X3grad = X3grad + beda/2 * sign(X3);
    Y3grad = Y3grad + gama/2 * sign(Y3);
